...
void WorldWindow::UpdateInput() {
    ...
    if (input.isCursorHovering(&windowShowRect) || relativeMouseMode) {
        if (input.isMouseButtonPressed(SDL_BUTTON_MIDDLE)) {
            ...
            camera->addViewportXY(-x * windowAreaScale,-y * windowAreaScale);
        }
            ...
            if (dir < 0) {
                windowAreaScale = min(windowAreaScale * 1.1f, 30.0f);
            }
            else {
...
void WorldWindow::DrawInfo() {
    ...
    texture = textRenderer.getTextTexture(
        "windowShowRect = {" +
        to_string((int)windowShowRect.x) + " " +
        to_string((int)windowShowRect.y) + " " +
        to_string((int)windowShowRect.w) + " " +
        to_string((int)windowShowRect.h) + " " + "}",
        "pingfang", 20);
    textRenderer.renderText(p.x, p.y, texture.get());
}
...
WorldWindow::WorldWindow(SDL_FRect& rect) : CreatorComponent(rect) {
    ...
    camera->setCameraRange({});
    BaseViewport = windowShowRect;
